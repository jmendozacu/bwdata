...
class Orders extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getOrderStatusData()
    {
        ...
        $model->getSelect()->joinLeft(['statustable' => $model->getTable('sales_order_status_state')],'main_table.status=statustable.status',['*']);
        $model->getSelect()->where("statustable.visible_on_front=1");
