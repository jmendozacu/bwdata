...
class Productlist extends \Magento\Framework\View\Element\Template
{
    ...
    {
        $visibilityAttrId = $this->eavAttribute->getIdByCode('catalog_product', 'visibility');
        ...
        if (!$this->_productlists) {
            ...
            if ($filterStatus) {
                $storeCollection->getSelect()->where(
                    ...
                    cpei.attribute_id = '.$proStatusAttId.' AND '.
            ...
            $storeCollection->getSelect()->join(
                $catalogProductEntityInt.' as cpei_vis',
                'main_table.mageproduct_id = cpei_vis.entity_id'
            );

            $storeCollection->getSelect()->where('cpei_vis.store_id='.$storeId.' AND cpei_vis.value=4 AND cpei_vis.attribute_id='.$visibilityAttrId);
            ...
            $adminStoreCollection->getSelect()->where('cpei_vis.store_id=0 AND cpei_vis.value=4 AND cpei_vis.attribute_id='.$visibilityAttrId);
            if ($from && $to) {
                $adminStoreCollection->getSelect()->where(
                    "cpe.created_at BETWEEN '".$from."' AND '".$to."'"
                );
