...
class SaveProduct {
    ...
    \Magento\Framework\Event\Manager $eventManager, \Magento\Framework\ObjectManagerInterface $objectManager, \Magento\Framework\Stdlib\DateTime\DateTime $date, \Magento\Framework\Stdlib\DateTime\Filter\Date $dateFilter, \Magento\Catalog\Model\Product\TypeTransitionManager $catalogProductTypeManager, \Magento\ConfigurableProduct\Model\Product\VariationHandler $variationHandler, \Magento\ConfigurableProduct\Model\Product\Type\Configurable $productTypeConfigurable, \Magento\Catalog\Api\ProductRepositoryInterface $productRepositoryInterface, \Magento\Catalog\Api\Data\ProductLinkInterfaceFactory $productLinkFactory, \Magento\Catalog\Model\Product\Link\Resolver $linkResolver, \Magento\Catalog\Model\Product\Initialization\Helper\ProductLinks $productLinks, \Magento\Backend\Helper\Js $jsHelper, \Magento\Framework\Filesystem $filesystem, Builder $marketplaceProductBuilder, MarketplaceHelperData $marketplaceHelperData, ProductApiHelper $productApiHelper
   ,\Webkul\Marketplace\Helper\Data $marketplacehelper ) {
    ...
    public function saveProductData($sellerId, $wholedata) {
        ...
        $wholedata['product']['attributproductBuildere_set_id'] = $wholedata['set'];
        $helper = $this->_marketplaceHelperData;
        $sellerId = $sellerId;

        /**
         * Single image for all simple products
         * adding the main product image for all the variations.
         */
        if (isset($wholedata['variations-matrix']) && is_array($wholedata['variations-matrix'])) {
            $newConfVariations = [];
            foreach ($wholedata['variations-matrix'] as $confVariations) {
                if (isset($confVariations['media_gallery'])) {
                    unset($confVariations['media_gallery']);
                }
                if (isset($wholedata['product']['image']) &&
                    isset($wholedata['product']['small_image']) &&
                    isset($wholedata['product']['thumbnail'])) {
                    unset($confVariations['image']);
                    unset($confVariations['small_image']);
                    unset($confVariations['thumbnail']);
                }
                $newConfVariations[] = $confVariations;
            }
            $wholedata['variations-matrix'] = $newConfVariations;
        }
        ...
        if (!empty($wholedata['id'])) {
            ...
            $this->_eventManager->dispatch(
                    'mp_customattribute_deletetierpricedata', [$wholedata]
            );
            ...
            /**
             * Setting url key for bakeway products
             */
            $wholedata['product']['url_key'] = uniqid(rand(), true);
        }
        if ($mageProductId) {
            /*
             * updating flag for edit request field 
             * unapproved status needs to flag only for image updating 
             * start code
             */
            $_ProductId = $wholedata['product_id'];
            /* get current product status */
            $prodStatus = $helper->getProductStatus($_ProductId);
            $wholedata['store'] = $storeId;
            $catalogProduct = $this->_productRepositoryInterface
                    ->getById(
                    $_ProductId, false, $storeId
            );

            //$_PostImages = count($wholedata['product']['media_gallery']['images']);
            $_OldImage = $catalogProduct->getImage();
            $_NewImage = $wholedata['product']['image'];
            if ($_OldImage != $_NewImage) {
         ...
         */
        $this->_eventManager->dispatch(
                'mp_product_save_before', [$wholedata]
        );
        ...
        $catalogProduct = $this->productInitialize(
                $this->_marketplaceProductBuilder->build($wholedata, $storeId), $wholedata
        );
        ...
        if (isset($wholedata[$catalogProduct->getIdFieldName()])) {
        ...
        /* 
         * check child product is configurable varients or not 
         * start
         */
        
        
         $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/checkconfilog.log');
        ...
        $parentIds = $this->_objectManager->get('Magento\ConfigurableProduct\Model\Product\Type\Configurable')
                ->getParentIdsByChild($catalogProduct->getId());
        if (isset($parentIds[0])) {
              $catalogProduct->setVisibility(Visibility::VISIBILITY_NOT_VISIBLE);
              $catalogProduct->save(); 
        }
        /*
         * end
        ...
        /**
         * Setting the SKU Dynamically Start
         * Setting SKU for Bakeway
         */
        $catalogProduct->setSku($sellerId . "-" . $catalogProduct->getId());
         ...
         */
        $this->productApiHelper->createProductCustomOptions($catalogProduct->getId());

        foreach ($associatedProductIds as $proId) {
            if ($proId) {
                $assProd = $this->_objectManager->create(
                                'Magento\Catalog\Model\Product'
                        )->load($proId);
                $assProd->setSku($sellerId . "-" . $assProd->getId());
                $assProd->setStockData([
                    'use_config_manage_stock' => 0,
                    'manage_stock' => 0,
                    'is_in_stock' => 1]);
                $assProd->setImage($catalogProduct->getImage());
                $assProd->setSmallImage($catalogProduct->getSmallImage());
                $assProd->setThumbnail($catalogProduct->getThumbnail());
                $assProd->setQuantityAndStockStatus(['manage_stock' => 0, 'qty' => 0, 'is_in_stock' => 1, 'use_config_manage_stock' => 0]);
                $assProd->save();
            }
        }
        /*         * ** SKU generation Ends *** */
        ...
        $this->_eventManager->dispatch(
                'mp_customoption_setdata', [$wholedata]
        );

        /* Update marketplace product */
        $this->saveMaketplaceProductTable(
                $mageProductId, $sellerId, $status, $editFlag, $associatedProductIds
        );
         ...
         */
        $this->_eventManager->dispatch(
                'mp_customattribute_settierpricedata', [$wholedata]
        );
         ...
         */
        $this->_eventManager->dispatch(
                'mp_product_save_after', [$wholedata]
        );
    ...
    private function productInitialize(\Magento\Catalog\Model\Product $catalogProduct, $requestData) {
        ...
        $catalogProduct->setCanSaveCustomOptions(
                (bool) $requestData['affect_product_custom_options'] && !$catalogProduct->getOptionsReadonly()
        );
        ...
        if (!empty($wholedata['attributes'])) {
            ...
            if (!empty($variationsMatrix)) {
                $generatedProductIds = $this->_variationHandler->generateSimpleProducts(
                        $catalogProduct, $variationsMatrix
                );
    ...
    $mageProductId, $sellerId, $status, $editFlag, $associatedProductIds
        ...
        if ($mageProductId) {
             ...
             */
            $catalogProduct = $this->_objectManager
                    ->create('Magento\Catalog\Model\Product')
                    ->load($mageProductId);
            $urlKey = $this->productApiHelper->createProductUrl($mageProductId);
        ...
        foreach ($associatedProductIds as $associatedProductId) {
            if ($associatedProductId) {
                ...
                $collection1->setSellerId($sellerId);
                $collection1->save();
                $assCatalogProduct = $this->_objectManager
                        ->create('Magento\Catalog\Model\Product')
                        ->load($mageProductId);
                $urlKey = $this->productApiHelper->createProductUrl($associatedProductId);
                $assCatalogProduct->setUrlKey($urlKey);
            }
        }
    }
                ...
                if (empty($customOptionData['is_delete'])) {
                    if (isset($customOptionData['values'])) {
                        $customOptionData['values'] = array_filter(
                                $customOptionData['values'], function ($valueData) {
    ...
    private function sendProductMail($data, $sellerId, $editFlag = null) {
        ...
        if (($editFlag == null && $helper->getIsProductApproval() == 1) || ($editFlag && $helper->getIsProductEditApproval() == 1)) {
            $this->_objectManager->create(
                    'Webkul\Marketplace\Helper\Email'
            )->sendNewProductMail(
                    $emailTempVariables, $senderInfo, $receiverInfo, $editFlag
            );
        }
    }
