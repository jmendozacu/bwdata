...
class MassDisapprove extends \Magento\Backend\App\Action {
    ...
    public function execute() {
        ...
        $sellerProduct->setProductData(
                $coditionData, ['status' => $status, 'seller_pending_notification' => 1]
        );
        ...
        foreach ($collection as $item) {
            ...
            $this->_objectManager->create(
                    ...
                    $emailTemplateVariables, $senderInfo, $receiverInfo
            );

            /* email id and name for getting alert of disapprove product */
            /* start */
            ...
            $dissStoremail = $helper->getDissProEmailId();
            if(!empty($dissStoremail)):
            $dissStoename = $helper->getDissProName();
            ...
            $this->_objectManager->get('Webkul\Marketplace\Helper\Email')->sendDisApprovedProEmail($all_EmailAddress,$emailTemplateVariables, $senderInfoCustom);
            endif;
            /* end */
            $this->_eventManager->dispatch(
                    'mp_disapprove_product', ['product' => $pro, 'seller' => $seller]
            );
        }

        $this->messageManager->addSuccess(
                __(
                        'A total of %1 record(s) have been disapproved.', $collection->getSize()
                )
        );
