    ...
    return Component.extend({
        ...
        generateGrid: function (variations, getSectionValue) {
            ...
            _.each(variations, function (options) {
                ...
                var bakeway_attribute_code = 'cake_weight';
                var multiplier = 1;
                _.each(options, function (option) {
                    if (option.attribute_code == bakeway_attribute_code) {
                        var optionLabelString = option.label.toLowerCase();
                        var labelArr = optionLabelString.split("kg");
                        if (labelArr.length > 0) {
                            if (!isNaN(parseFloat(labelArr[0]))){
                                multiplier = parseFloat(labelArr[0]);
                            } else {
                                multiplier = 0;
                            }
                        }
