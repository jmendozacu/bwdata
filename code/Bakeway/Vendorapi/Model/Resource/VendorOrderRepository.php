...
use Bakeway\OrderstatusEmail\Model\Email as StatusEmail;
...
use Bakeway\Razorpay\Helper\Data as RazorpayHelper;
/**
...
class VendorOrderRepository implements VendorOrderRepositoryInterface {
     ...
     * @param StatusEmail $statusEmail
     * @param ObjectManagerInterface $objectManagerInterface
     * @param GrabintigrationHelper $grabintigrationHelper
    ...
    public function acceptOrder($vendorId, $orderId) {
        ...
        if ($vendorOrderId == $orderId) {
            ...
            if ($order->getStatus() == "pending"):
                ...
                $order->save();
                /**
                 * function for capturing order payment only if the payment method is razorpay
                 */
                 ...
                 */
                //$this->statusEmail->accpetUserEmail($orderId);
    ...
    public function rejectOrder($vendorId, $orderId, $reason = null) {
        ...
        if ($vendorOrderId == $orderId) {
            ...
            if ($order->getStatus() == "pending"):
                ...
                $order->save();
                $this->statusEmail->rejectUserEmail($orderId);
                /**
                 * Paytm payment Refund
                 * commenting code for refund as per BKWYADMIN-744
                 */
                /*$paymentMethod = $order->getPayment()->getMethod();

                if ($paymentMethod == 'paytm') {
                    $this->paytmHelper->initiatePaytmRefund($order);
                } elseif ($paymentMethod == 'razorpay') {
                    $this->razorpayHelper->initiateRazorpayRefund($order);
                }*/
                return $order;
            else:
                throw new CouldNotSaveException(__('This opration was not processed.Please refresh for Updated status'));
    ...
    public function setOrderStatus($vendorId, $orderId, $status_code) {
        ...
        if ($vendorOrderId == $orderId) {
            ...
            if (in_array($status_code, $actionStatuses)) {
                ...
                $serverDtime = $this->objectManagerInterface->create('\Magento\Framework\Stdlib\DateTime\TimezoneInterface');
                ...
                if ($check === true):
                    if ($status_code == \Magento\Sales\Model\Order::STATE_COMPLETE) {
                        ...
                        throw new  CouldNotSaveException(__('Could not complete the order before '.$timetext.' time.'));
                        endif;
                    } else {
                    ...
                    if ($status_code == BakewayOrderStatus::STATUS_ORDER_OUT_FOR_DELIVERY):
                        ...
                        //$this->statusEmail->sendOrderOutForDeliveryStatusEmailToUser($orderId);
                        /**
                         * grab code start
                         */
                        $grabDeliveryFlag = $order->getData('grab_delivery_flag');

                        if(!empty($grabDeliveryFlag)):
                            $this->grabintigrationHelper->outForDeliveryStatusGrabApiCall($order,$vendorId);
                        endif;

                    elseif ($status_code == BakewayOrderStatus::STATUS_ORDER_READY):

                        //$this->statusEmail->sendOrderReadyforpickupStatusEmailToUser($orderId);

                    elseif ($status_code == \Magento\Sales\Model\Order::STATE_COMPLETE):
                        if($completeCheck):
                         $this->statusEmail->sendOrderCompleteStatusEmailToUser($orderId);
                        endif;
                    endif;
                    return $order;
                endif;
    ...
    public function getVendorUnpaidOrderList($vendorId) {
        ...
        $orderCollection->getSelect()->joinInner([
            'mp_order' => $orderCollection->getTable('marketplace_orders')], 'main_table.entity_id=mp_order.order_id', [
            'payment_gateway_fee', 'delivery_fee', 'tcs_amount', 'coupon_amount',
            'convenience_fee_incl_tax', 'tax_paid_by_bakeway']);
        ...
        foreach ($orderCollection as $order) {
            ...
            if ($isShipping) {
                ...
                $totalAmount = $totalAmount - $lastCouponAmount;
        ...
        $transCollection = $this->sellerTransactionFactory->create()
                        ->addFieldToFilter("seller_id", $vendorId)
                        ->addFieldToFilter("transaction_status", 1)
     ...
     */
    public function getRejectedReasonsList($vendorId,$device = null) {
        ...
        if($device == 'web'){
            $query = 'SELECT id,reason_content FROM `' . $table . '` WHERE is_for_seller=1 AND is_active=1';
    ...
    public function vendoracceptOrder($vendorId, $orderId) {
        ...
        if ($vendorOrderId == $orderId) {
             ...
             */
            //$this->statusEmail->accpetUserEmail($orderId);
            $this->_messageManager->addSuccess('Order accepted by partner.');
            $this->eventManager->dispatch(
                    'bakeway_vendor_order_accept_after', ['order' => $order]
            );
            return $order;
    ...
    public function vendorsetOrderStatus($vendorId, $orderId, $status_code) {
        ...
        if ($vendorOrderId == $orderId) {
            if (in_array($status_code, $actionStatuses)) {
                ...
                $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/ordercheck.log');
