...
namespace Bakeway\ReviewRating\Block\Adminhtml\Rating\Edit;
    ...
    protected function _prepareForm()
    {
       ...
       // $fieldset->addField('customer', 'note', ['label' => __('Author'), 'text' => $customerText]);
        ...
        $fieldset->addField(
            ...
            [
                'label' => __('Order Id'),
                'text' => ($review['order_id'] ? $review['order_id']  : 'NA')
            ]
        );

        if($review['order_review_status']) {
            if($review['order_review_status'] == 1){
                $orderStatus = \Bakeway\ReviewRating\Helper\Data::SUBMITTED_STATUS_STRING;
            }else{
                $orderStatus = \Bakeway\ReviewRating\Helper\Data::PENDING_STATUS_STRING;
            }
            $fieldset->addField(
                'order_review_status',
                'note',
                [
                    'label' => __('Order Feedback Status'),
                    'text' => $orderStatus
                ]
            );
        }
