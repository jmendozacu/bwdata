...
class PaymentgatewayRepository implements PaymentgatewayRepositoryInterface {
    ...
    /**
     * \Bakeway\Razorpay\Helper\Data $razorPayHelper
     */
     ...
     * @param \Bakeway\Razorpay\Helper\Data $razorPayHelper
    ...
    public function order($order_id, $cartId) {
        ...
        $deliveryTime = '';
        $deliveryType = '';
        if ($quoteObj->getData('delivery_type')) {
            $deliveryType = $quoteObj->getData('delivery_type');
        }
        if ($quoteObj->getData('delivery_time')) {
            $deliveryTime = $quoteObj->getData('delivery_time');
        }
        $this->customAttributeHelper->applyQuoteValidations($quoteObj, $deliveryType, $deliveryTime, null, true);
        ...
        try {
            $order = $rzpmainobj->order->create([
                ...
                'payment_capture' => 1 // auto-capture set 1 when needs auto capture
            ]);
    ...
    public function processcapture($cartId, $razorpay_payment_id, $razorpay_order_id, $razorpay_signature) {
        ...
        $checkOrderId = $this->razorPayHelper->confirmOrderId($cartId,$razorpay_order_id);
        if($checkOrderId === true) {
            if (empty($razorpay_payment_id) === false) {
                ...
                try {
                    ...
                    $quoteObj->save();
                    /*
                    * adding log @12-01-2018
                    */
                    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/razorpay_singnaturepass.log');
                    $logger = new \Zend\Log\Logger();
                    $logger->addWriter($writer);
                    $logger->info('signature ' . $razorpay_payment_id);

                } catch (SignatureVerificationError $e) {
                    /*
                     * adding log @12-01-2018
                     */
                    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/razorpay_singnaturefailure.log');
                    $logger = new \Zend\Log\Logger();
                    $logger->addWriter($writer);
                    $logger->info("error");
                    $success = false;
    ...
    public function processcaptureguest($cartId, $razorpay_payment_id, $razorpay_order_id, $razorpay_signature) {
        ...
        if(isset($cartId))
        {
            ...
            throw new NotFoundException(__('Cart id is empty'));
        }
