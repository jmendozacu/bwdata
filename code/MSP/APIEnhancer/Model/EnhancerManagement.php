...
class EnhancerManagement implements EnhancerManagementInterface
{
    ...
    public function canCacheRequest()
    {
        ...
        if (preg_match('|' . '/rest/V1/partners\?' . '|i', $uriPath)) {
            return true;
        }

        if (preg_match('|' . '/rest/V1/partners/[0-9]+/products\?' . '|i', $uriPath)) {
            return true;
        }

        if (preg_match('|' . '/rest/V1/category/[0-9]+/products\?' . '|i', $uriPath)) {
            return true;
        }

        if (preg_match('|' . 'rest/V1/partners/filters' . '|i', $uriPath)) {
            return true;
        }
